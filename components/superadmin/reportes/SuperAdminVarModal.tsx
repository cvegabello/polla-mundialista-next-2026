...
export const SuperAdminVarModal = ({
  isOpen,
  onClose,
  lang,
}: SuperAdminVarModalProps) => {
  const [loading, setLoading] = useState(false);
  const [isRefreshing, setIsRefreshing] = useState(false);
  const [data, setData] = useState<any>(null);

  const [pollas, setPollas] = useState<any[]>([]);
  const [selectedPollaId, setSelectedPollaId] = useState<string>("");
  ...
  const fetchPollasList = useCallback(async () => {
    const res = await getAllPollasAction();
    if (res.success && res.data) {
      setPollas(res.data);
      if (res.data.length > 0) {
        setSelectedPollaId(res.data[0].id);
      }
    }
  }, []);

  useEffect(() => {
    if (isOpen) fetchPollasList();
  }, [isOpen, fetchPollasList]);

  const loadData = useCallback(async (pollaId: string, showMainLoader = true) => {
    if (!pollaId) return;
    if (showMainLoader) setLoading(true);
    else setIsRefreshing(true);

    const result = await getSuperAdminVarReportAction(pollaId);
  ...
  useEffect(() => {
    if (isOpen && selectedPollaId) {
      loadData(selectedPollaId, true);
    }
  }, [isOpen, selectedPollaId, loadData]);
          ...
          {isNoneExpanded && (
            <div className="flex flex-col lg:flex-row lg:justify-between items-center p-4 pt-16 lg:p-5 lg:pt-5 border-b border-orange-600 bg-[#050200]/80 backdrop-blur-sm shrink-0 gap-4 lg:gap-0 relative z-10">
              ...
              <div className="flex flex-nowrap justify-center items-center gap-2 sm:gap-3 shrink-0">
                {/* 🏆 COMBOBOX DE POLLAS PARA SUPER ADMIN */}
                <div className="flex items-center bg-black/80 backdrop-blur-sm border border-purple-600 rounded-lg overflow-hidden focus-within:border-purple-400 focus-within:shadow-[0_0_10px_rgba(168,85,247,0.3)] transition-all">
                  <span className="pl-2 sm:pl-3 text-purple-500 text-sm">
                    🏆
                  </span>
                  <select
                    value={selectedPollaId}
                    onChange={(e) => setSelectedPollaId(e.target.value)}
              ...
              <div className="absolute top-4 right-4 lg:static lg:top-auto lg:right-auto flex justify-end gap-2 shrink-0 lg:ml-4">
                <button
                  onClick={() => loadData(selectedPollaId, false)}
                                ...
                                return (
                                  <React.Fragment key={`${user.id}-${m.id}`}>
                                    <td className="px-2 py-2 border-r border-orange-900/40 font-mono text-gray-300 text-xs text-center">
                                      {pred?.pred_home ?? "-"}
                                      {pred?.pred_home != null && pred?.pred_home === pred?.pred_away && pred?.predicted_winner && pred?.predicted_winner === m.home?.id && (
                                        <span className="ml-1 text-[10px]" title="Ganador en Penales">⭐</span>
                                      )}
                                    </td>
                                    <td className="px-2 py-2 border-r border-orange-900/40 font-mono text-gray-300 text-xs text-center">
                                      {pred?.pred_away ?? "-"}
                                      {pred?.pred_away != null && pred?.pred_home === pred?.pred_away && pred?.predicted_winner && pred?.predicted_winner === m.away?.id && (
                                        <span className="ml-1 text-[10px]" title="Ganador en Penales">⭐</span>
                                      )}
                                    </td>
